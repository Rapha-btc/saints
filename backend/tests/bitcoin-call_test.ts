...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const contractAddressCall = deployer.address + 'bitcoin-call';
...
Tx.contractCall('bitcoin-call', 'mint', [types.uint(2100), types.uint(1000000000), types.uint(15000000)], deployer.address),
            // Transfer u1 u3 and u5 to wallet 1
Tx.contractCall('bitcoin-call', 'transfer', [types.uint(1), types.principal(deployer.address), types.principal(wallet1.address)], deployer.address),
Tx.contractCall('bitcoin-call', 'transfer', [types.uint(3), types.principal(deployer.address), types.principal(wallet1.address)], deployer.address),
Tx.contractCall('bitcoin-call', 'transfer', [types.uint(5), types.principal(deployer.address), types.principal(wallet1.address)], deployer.address)
        ...
        // exercise u5
Tx.contractCall('bitcoin-call', 'exercise', [types.uint(5)], wallet1.address)
 // where is the Xbtc contract?
        ...
        // reclaim u2 baby!
// Tx.contractCall('bitcoin-call', 'reclaim-yy', [types.uint(2), "(ok true)"], deployer.address)
// not able to run the line above?
Tx.contractCall('bitcoin-call', 'counterparty-reclaim', [types.uint(2)], deployer.address)
        ]); 
...
Tx.contractCall('bitcoin-call', 'transfer', [types.uint(5), types.principal(deployer.address), types.principal(wallet1.address)], deployer.address)
        ...
        let block2 = chain.mineBlock([
        // exercise u5
Tx.contractCall('bitcoin-call', 'exercise-all-of-my-exerciser-calls', [], wallet1.address)
 // where is the Xbtc contract?
        ]); 
        ...
        chain.mineEmptyBlock(2100); // options expired now!
        let block3 = chain.mineBlock([
Tx.contractCall('bitcoin-call', 'counterparty-reclaim', [types.uint(1)], deployer.address)
]); 
...
Tx.contractCall('bitcoin-call', 'exercise-all-of-my-exerciser-calls', [], wallet1.address)
 // where is the Xbtc contract?
        ...
        let deployerReclaim = chain.callReadOnlyFn('bitcoin-call', 'get-reclaimable-calls', [types.principal(deployer.address)], deployer.address);
...
Tx.contractCall('bitcoin-call', 'transfer-same-strikes', [myList, types.principal(deployer.address), types.principal(wallet1.address)], deployer.address),
Tx.contractCall('bitcoin-call', 'exercise-all-of-my-exerciser-calls', [], wallet1.address)
        ...
        let block3 = chain.mineBlock([
Tx.contractCall('bitcoin-call', 'reclaiming', [], deployer.address)
]); 
...
//         let block2 = chain.mineBlock([
//         // exercise u5
// Tx.contractCall('bitcoin-call', 'exercise-all-of-my-exerciser-calls', [types.principal(contractAddress)], wallet1.address)
//  // where is the Xbtc contract?
//         ]); 
//         console.log("printing exercising u1........", block2.receipts[0].events);
        // console.log("printing exercising u3........", block2.receipts[1].events);
        ...
        // chain.mineEmptyBlock(2100); // options expired now!
//         let block3 = chain.mineBlock([
// Tx.contractCall('bitcoin-call', 'counterparty-reclaim', [types.principal(contractAddress), types.uint(1)], deployer.address)
// ]); 
//         block3.receipts[0].result.expectErr().expectUint(1012); // deployer cannot reclaim it as it desn't belong to him and it's not expired
// },
// });
